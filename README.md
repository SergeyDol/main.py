...
Проект для работы с банковскими транзакциями: чтение, фильтрация, сортировка и анализ.

## Новая функциональность

### 1. Поиск транзакций по описанию

Реализована функция `process_bank_search` в модуле `utils.py`, которая:
- Принимает список транзакций и строку для поиска
- Использует библиотеку `re` для поиска с регулярными выражениями
- Возвращает список транзакций, содержащих искомую строку в описании

### 2. Подсчет операций по категориям

Реализована функция `process_bank_operations` в модуле `utils.py`, которая:
- Принимает список транзакций и список категорий
- Использует `Counter` из библиотеки `collections` для подсчета
- Возвращает словарь с количеством операций по категориям

### 3. Основной интерфейс

Реализована функция `main` в модуле `main.py`, которая предоставляет:
- Интерактивный интерфейс для работы с транзакциями
- Чтение данных из JSON, CSV и Excel файлов
- Фильтрацию по статусу, валюте, описанию
- Сортировку по дате
